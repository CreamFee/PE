...
public class Funcion1 {
    ...
    public double mutar;
    public double cruce;
    private double precision = 0.001; //Solicitar
    private Cromosoma[] individuos;
    private boolean tipoCruce;
    public Funcion1(int poblacion, double precision, double mutar, double cruce, boolean tipoCruce){
        this.sizeCrom = 0;
        this.gens = 2;
    ...
    private void init(){ //TODO
        ...
        for(int i = 0; i < this.poblacion; i++){
            this.individuos[i] = new Cromosoma (this.sizeCrom, this.gens, this.ranges); //Creamos todos los individuos
            this.individuos[i].init(); //Arrancamos la poblacion aleatoria
        }

    };
    private double evaluate (Cromosoma c){ //DONE
        //Evalua 1 cromosoma
    ...
    public int calcSizeGen (double precision, double range0, double range1){ //DONE
    ...
    public double getMax(){ //Devuelve el maximo de esta poblacion
        double max = 0, tmp = 0;
        for (int i = 0; i < poblacion; i++){
    ...
    public double getAverage(){ //Calcula la media aritmetica
    ...
    public void seleccionar(int[] seleccion) {
    	...
    	for(int i = 0; i < poblacion; i++) {
    		nuevos[i] = individuos[seleccion[i]];
    	}
    	for(int i = 0; i < poblacion; i++) {
